...
export default function DiseaseGraph() {
    ...
    useEffect(() => {
        ...
        const COLOR_HEALTHY = styles.getPropertyValue('--color-primary-blue').trim() || '#78a0ff';
        const COLOR_INFECTED = styles.getPropertyValue('--color-accent-red').trim() || '#f05464';
        const COLOR_REMOVED = styles.getPropertyValue('--color-gray-dark').trim() || '#505050';
        ...
        class Transmission {
            ...
            update() {
                ...
                if (this.startNode.state === 'removed' || this.endNode.state === 'removed') {
        ...
        const animate = () => {
                ...
                for (let j = i + 1; j < nodes.length; j++) {
                    ...
                    if (dist < CONNECTION_DISTANCE) {
                        ...
                        if (nodeA.state === 'infected' && nodeB.state === 'healthy') {
                            source = nodeA;
                            target = nodeB;
                        } else if (nodeB.state === 'infected' && nodeA.state === 'healthy') {
                            source = nodeB;
                            target = nodeA;
                        }

                        if (source && target) {
                            const alreadyTransmitting = transmissions.some(t =>
                                (t.startNode === source && t.endNode === target)
                            );
            ...
            if (!nodes.some(n => n.state === 'infected')) {
                ...
                if (healthyNodes.length > 0) {
                    healthyNodes[Math.floor(Math.random() * healthyNodes.length)].state = 'infected';
                }
            }
